...
def main(server, api, cert, csvfile, delete):
    ...
    for chunks in getChunks(data, 5):
        ...
        for row in chunks:
            ...
            if delete:
                ...
                result = tak.deleteUser(api, cert, user)
                if result.status_code == 200:
                    print(f"User '{user}' deleted")
            elif tak.userExists(api, cert, user):
                ...
                result = tak.createUser(
                    api, cert, user, password, groups, groupsIn, groupsOut
                )
                if result.status_code == 200:
        ...
        pdf.add_section(
            Section(text),
            user_css="h1 {font-size: 1.5em;} table {border-bottom: 1px solid black;} tr {width: 100%} td {padding: 0.5em;} .secret {color: red; font-size: 1em; font-weight: bold;} .creds {padding: 1em;} .instructions {font-size: 0.75em;} .group {font-size: 0.5em;}",
        )
...
def configParser():
    ...
    options = "hf:s:c:k:"
    long_options = ["Help", "File=", "Server=", "Cert=", "Key=", "delete"]
    server = None
    cert = None
    key = None
    csvfile = None
    ...
    try:
        ...
        for currentArgument, currentValue in arguments:
            ...
            elif currentArgument in ("--delete"):
                delete = True
    ...
    crt = (cert, key)
    return server, crt, csvfile, delete


server, cert, csvfile, delete = configParser()
...
if server != "" and len(cert) > 0 and csvfile != None:
    ...
    if os.path.exists(csvfile):
        ...
        if tak.isAdmin(api, cert):
            print("User is admin")
            main(server, api, cert, csvfile, delete)
        else:
